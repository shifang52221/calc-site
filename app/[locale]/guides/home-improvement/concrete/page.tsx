...
export async function generateMetadata({
  ...
  const t = await getTranslations("guideConcrete");

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: getAlternates(locale, "/guides/home-improvement/concrete"),
  };
}

export default async function ConcreteGuidePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const breadcrumbs = [
    ...
    {
      name: cards("concreteTitle"),
      url: getLocalizedUrl(locale, "/guides/home-improvement/concrete"),
    },
  ];
  ...
  return (
    <div className="mx-auto grid max-w-3xl gap-8">
      ...
      <section className="grid gap-3 rounded-xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950">
        <h2 className="text-base font-semibold">{t("formulaTitle")}</h2>
      ...
      <GuideRelatedSection locale={locale} guideId="concrete" />
