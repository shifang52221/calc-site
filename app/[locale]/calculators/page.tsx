...
export default async function CalculatorsIndex({
  ...
  const initialCategoryId =
    ...
    filterReviewVisibleCategories(
      ...
      CALCULATORS,
      locale,
    ).some((category) => category.id === rawCategory)
      ? (rawCategory as (typeof CALCULATOR_CATEGORIES)[number]["id"] | "all")
      : "all";

  const visibleCategories = filterReviewVisibleCategories(
    CALCULATOR_CATEGORIES,
    CALCULATORS,
    locale,
  );
  const calculators = sortReviewVisibleCalculators(CALCULATORS, locale).map(
    (calculator) => ({
