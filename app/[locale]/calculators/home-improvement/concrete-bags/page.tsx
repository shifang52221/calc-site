...
import { ConcreteBagsCalculator } from "@/calculators/concrete-bags/ConcreteBagsCalculator";
...
export async function generateMetadata({
  ...
  const t = await getTranslations("concreteBags");

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: getAlternates(locale, "/calculators/home-improvement/concrete-bags"),
    robots: isReviewNoindexCalculator(locale, "concreteBags")
      ? { index: false, follow: true }
      : undefined,
  };
}

export default async function ConcreteBagsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="grid gap-8">
      <CalculatorSeoJsonLd
        locale={locale}
        pathname="/calculators/home-improvement/concrete-bags"
        title={t("metaTitle")}
        description={t("metaDescription")}
        faq={faq}
      />
      ...
      <CalculatorContent locale={locale} calculatorId="concreteBags" variant="before" />

      <ConcreteBagsCalculator />

      <AdSlot slot={ADSENSE_SLOTS.calculatorAfterResult} />

      <CalculatorContent locale={locale} calculatorId="concreteBags" variant="after" />
      ...
      <CalculatorRelatedSection locale={locale} calculatorId="concreteBags" />
