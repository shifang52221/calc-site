...
import { MulchBagsCalculator } from "@/calculators/mulch-bags/MulchBagsCalculator";
...
export async function generateMetadata({
  ...
  const t = await getTranslations("mulchBags");

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: getAlternates(locale, "/calculators/home-improvement/mulch-bags"),
    robots: isReviewNoindexCalculator(locale, "mulchBags")
      ? { index: false, follow: true }
      : undefined,
  };
}

export default async function MulchBagsPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="grid gap-8">
      <CalculatorSeoJsonLd
        locale={locale}
        pathname="/calculators/home-improvement/mulch-bags"
        title={t("metaTitle")}
        description={t("metaDescription")}
        faq={faq}
      />
      ...
      <CalculatorContent locale={locale} calculatorId="mulchBags" variant="before" />

      <MulchBagsCalculator />

      <AdSlot slot={ADSENSE_SLOTS.calculatorAfterResult} />

      <CalculatorContent locale={locale} calculatorId="mulchBags" variant="after" />
      ...
      <CalculatorRelatedSection locale={locale} calculatorId="mulchBags" />
