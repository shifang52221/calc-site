...
import { GravelTonsCalculator } from "@/calculators/gravel-tons/GravelTonsCalculator";
...
export async function generateMetadata({
  ...
  const t = await getTranslations("gravelTonsCalc");

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: getAlternates(locale, "/calculators/home-improvement/gravel-tons"),
    robots: isReviewNoindexCalculator(locale, "gravelTons")
      ? { index: false, follow: true }
      : undefined,
  };
}

export default async function GravelTonsCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="grid gap-8">
      <CalculatorSeoJsonLd
        locale={locale}
        pathname="/calculators/home-improvement/gravel-tons"
        title={t("metaTitle")}
        description={t("metaDescription")}
        faq={faq}
      />
      ...
      <CalculatorContent locale={locale} calculatorId="gravelTons" variant="before" />

      <GravelTonsCalculator />

      <AdSlot slot={ADSENSE_SLOTS.calculatorAfterResult} />

      <CalculatorContent locale={locale} calculatorId="gravelTons" variant="after" />
      ...
      <CalculatorRelatedSection locale={locale} calculatorId="gravelTons" />
