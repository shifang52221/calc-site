...
export async function generateMetadata({
  ...
  const t = await getTranslations("paverBase");
...
export default async function PaverBasePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="grid gap-8">
      <CalculatorSeoJsonLd
        locale={locale}
        pathname="/calculators/home-improvement/paver-base"
        title={t("metaTitle")}
        description={t("metaDescription")}
        faq={faq}
      />
      ...
      <CalculatorContent locale={locale} calculatorId="paverBase" variant="before" />

      <PaverBaseCalculator />

      <AdSlot slot={ADSENSE_SLOTS.calculatorAfterResult} />

      <CalculatorContent locale={locale} calculatorId="paverBase" variant="after" />
      ...
      <CalculatorRelatedSection locale={locale} calculatorId="paverBase" />
