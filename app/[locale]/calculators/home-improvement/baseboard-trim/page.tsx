...
import { BaseboardTrimCalculator } from "@/calculators/baseboard-trim/BaseboardTrimCalculator";
...
export async function generateMetadata({
  ...
  const t = await getTranslations("baseboardTrim");

  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: getAlternates(locale, "/calculators/home-improvement/baseboard-trim"),
    robots: isReviewNoindexCalculator(locale, "baseboardTrim")
      ? { index: false, follow: true }
      : undefined,
  };
}

export default async function BaseboardTrimPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  return (
    <div className="grid gap-8">
      <CalculatorSeoJsonLd
        locale={locale}
        pathname="/calculators/home-improvement/baseboard-trim"
        title={t("metaTitle")}
        description={t("metaDescription")}
        faq={faq}
      />
      ...
      <CalculatorContent
        locale={locale}
        calculatorId="baseboardTrim"
        variant="before"
      />

      <BaseboardTrimCalculator />
      ...
      <CalculatorReferenceSection locale={locale} calculatorId="baseboardTrim" />

      <CalculatorRelatedSection locale={locale} calculatorId="baseboardTrim" />
